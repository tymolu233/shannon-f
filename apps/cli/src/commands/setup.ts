...
async function setupRouter(): Promise<ShannonConfig> {
  ...
  if (routerProvider === 'openai') {
    ...
    const customizeBaseUrl = await p.confirm({
      message: 'Do you want to override the default OpenAI API endpoint?',
      initialValue: false,
    });
    if (p.isCancel(customizeBaseUrl)) return cancelAndExit();

    if (customizeBaseUrl) {
      const baseUrl = await p.text({
        message: 'OpenAI-compatible chat completions URL',
        initialValue: 'https://api.openai.com/v1/chat/completions',
        validate: required('OpenAI-compatible endpoint URL is required'),
      });
      if (p.isCancel(baseUrl)) return cancelAndExit();
  ...
  if (routerProvider === 'openai') {
    ...
    if (openaiBaseUrl) {
      router.openai_base_url = openaiBaseUrl;
    }
  } else {
